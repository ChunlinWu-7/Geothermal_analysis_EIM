...
/*
	M1 functions: original
*/

complex<double> DIE::M1_ori()
{
	if (dist_flag != 0) {
		return M1_r_ori[0];
	}
	else {
		return 4.0 * pi / pow(beta, 3.0) * (
			-2.0i + exp(1.0i * a * beta) *
			((1.0 + 1.0i) - 1.0i * a * beta) *
			((1.0 + 1.0i) + a * beta)
			);
	}
}

complex<double> DIE::M1_ori_1(int i)
{
	if (dist_flag != 0) {
		return M1_r_ori[1] * psi_ori_1(i);
	}
	else {
		return 0.0 + 0.0i;
	}
...
complex<double> DIE::M1_ori_2(int i, int j)
{
	if (dist_flag != 0) {
		return M1_r_ori[1] * psi_ori_2(i, j) + M1_r_ori[2] * psi_ori_1(i) * psi_ori_1(j);
	}
	else {
		return d[i][j] * 4.0i / 3.0 * pi * beta * a * a * exp(1.0i * a * beta);
	}
}

complex<double> DIE::M1_ori_3(int i, int j, int k)
{
	if (dist_flag != 0) {
		return M1_r_ori[1] * psi_ori_3(i, j, k)
			+ M1_r_ori[2] * (
				psi_ori_1(k) * psi_ori_2(i, j)
				+ psi_ori_1(i) * psi_ori_2(j, k)
				+ psi_ori_1(j) * psi_ori_2(i, k)
				)
			+ M1_r_ori[3] * psi_ori_1(i) * psi_ori_1(j) * psi_ori_1(k);
	}
	else {
		return 0.0 + 0.0i;
	}
...
complex<double> DIE::M1_ori_4(int i, int j, int k, int l)
{
	if (dist_flag != 0) {
		return M1_r_ori[1] * psi_ori_4(i, j, k, l)
			+ M1_r_ori[2] * (
			...
			+ M1_r_ori[4] * psi_ori_1(i) * psi_ori_1(j) * psi_ori_1(k) * psi_ori_1(l);
	}
	else {
		return (d[i][j] * d[k][l] + d[i][k] * d[j][l] + d[i][l] * d[j][k]) * (
			-4.0i / 15.0 * exp(1.0i * a * beta) * pi * beta * (
				2.0 + a * beta * (-2.0i + a * beta)
				)
			);
	}
}

complex<double> DIE::M1_ori_5(int i, int j, int k, int l, int m)
{
	if (dist_flag != 0) {
		return M1_r_ori[1] * psi_ori_5(i, j, k, l, m)
			+ M1_r_ori[2] * psi_ori_4(i, j, k, l) * psi_ori_1(m)

			+ M1_r_ori[2] * (psi_ori_4(i, j, k, m) * psi_ori_1(l) + psi_ori_3(i, j, k) * psi_ori_2(l, m))
			+ M1_r_ori[3] * psi_ori_1(m) * (psi_ori_3(i, j, k) * psi_ori_1(l))


			+ (M1_r_ori[4] * psi_ori_1(l) * psi_ori_1(m) + M1_r_ori[3] * psi_ori_2(l, m)) * (
				psi_ori_1(k) * psi_ori_2(i, j)
				+ psi_ori_1(i) * psi_ori_2(j, k)
				+ psi_ori_1(j) * psi_ori_2(i, k)
				)

			+ M1_r_ori[3] * psi_ori_1(l) * (
			...
			+ M1_r_ori[5] * psi_ori_1(i) * psi_ori_1(j) * psi_ori_1(k) * psi_ori_1(l) * psi_ori_1(m);
	}
	else {
		return 0.0 + 0.0i;
	}
...
complex<double> DIE::M1_ori_6(int i, int j, int k, int l, int m, int n)
{
	if (dist_flag != 0) {
		complex<double> result = M1_r_ori[1] * psi_ori_6(i, j, k, l, m, n) + M1_r_ori[2] * psi_ori_1(n) * psi_ori_5(i, j, k, l, m)
			+ M1_r_ori[2] * psi_ori_5(i, j, k, l, n) * psi_ori_1(m) + M1_r_ori[2] * psi_ori_4(i, j, k, l) * psi_ori_2(m, n)
			+ M1_r_ori[3] * psi_ori_4(i, j, k, l) * psi_ori_1(m) * psi_ori_1(n);

		result += M1_r_ori[3] * psi_ori_1(n) * (psi_ori_4(i, j, k, m) * psi_ori_1(l) + psi_ori_3(i, j, k) * psi_ori_2(l, m))
			+ M1_r_ori[2] * (
			...
			+ M1_r_ori[4] * psi_ori_1(n) * psi_ori_1(m) * psi_ori_3(i, j, k) * psi_ori_1(l)
			+ M1_r_ori[3] * (
		...
		result += (M1_r_ori[5] * psi_ori_1(l) * psi_ori_1(m) * psi_ori_1(n) + M1_r_ori[4] * psi_ori_2(l, n) * psi_ori_1(m)
			+ M1_r_ori[4] * psi_ori_1(l) * psi_ori_2(m, n) + M1_r_ori[4] * psi_ori_1(n) * psi_ori_2(l, m) + M1_r_ori[3] * psi_ori_3(l, m, n))
			* (
				psi_ori_1(k) * psi_ori_2(i, j)
				+ psi_ori_1(i) * psi_ori_2(j, k)
				+ psi_ori_1(j) * psi_ori_2(i, k)
		...
		result += (M1_r_ori[4] * psi_ori_1(l) * psi_ori_1(n) + M1_r_ori[3] * psi_ori_2(l, n)) * (
			psi_ori_2(k, m) * psi_ori_2(i, j) + psi_ori_1(k) * psi_ori_3(i, j, m)
			+ psi_ori_2(i, m) * psi_ori_2(j, k) + psi_ori_1(i) * psi_ori_3(j, k, m)
			+ psi_ori_2(j, m) * psi_ori_2(i, k) + psi_ori_1(j) * psi_ori_3(i, k, m)
			) + M1_r_ori[3] * psi_ori_1(l) * (
		...
		result += (M1_r_ori[4] * psi_ori_1(n) * psi_ori_1(m) + M1_r_ori[3] * psi_ori_2(m, n)) * (
			psi_ori_2(k, l) * psi_ori_2(i, j) + psi_ori_1(k) * psi_ori_3(i, j, l)
			+ psi_ori_2(i, l) * psi_ori_2(j, k) + psi_ori_1(i) * psi_ori_3(j, k, l)
			+ psi_ori_2(j, l) * psi_ori_2(i, k) + psi_ori_1(j) * psi_ori_3(i, k, l)
			) + M1_r_ori[3] * psi_ori_1(m) * (
		...
		result += (M1_r_ori[5] * psi_ori_1(n) * psi_ori_1(m) + M1_r_ori[4] * psi_ori_2(m, n)) * (
			psi_ori_2(i, l) * psi_ori_1(j) * psi_ori_1(k)
			+ psi_ori_1(i) * psi_ori_2(j, l) * psi_ori_1(k)
			+ psi_ori_1(i) * psi_ori_1(j) * psi_ori_2(k, l)
			) + M1_r_ori[4] * psi_ori_1(m) * (
		...
		result += M1_r_ori[6] * psi_ori_1(i) * psi_ori_1(j) * psi_ori_1(k) * psi_ori_1(l) * psi_ori_1(m) * psi_ori_1(n)
			+ M1_r_ori[5] * (
		...
		complex<double> result = 4.0 / 105.0 * exp(1.0i * a * beta) * pi
			* pow(beta, 3.0) * (4.0i + a * beta * (4.0 + 1.0i * a * beta));
...
complex<double> DIE::M1()
{
	return M1_r[0];
}

complex<double> DIE::M1_1(int i)
{
	return M1_r[1] * psi_1(i);
}

complex<double> DIE::M1_2(int i, int j)
{
	return M1_r[1] * psi_2(i, j) + M1_r[2] * psi_1(i) * psi_1(j);
}

complex<double> DIE::M1_3(int i, int j, int k)
{
	return M1_r[1] * psi_3(i, j, k)
		+ M1_r[2] * (
			psi_1(k) * psi_2(i, j)
			+ psi_1(i) * psi_2(j, k)
			+ psi_1(j) * psi_2(i, k)
			)
		+ M1_r[3] * psi_1(i) * psi_1(j) * psi_1(k);
}

complex<double> DIE::M1_4(int i, int j, int k, int l)
{
	return M1_r[1] * psi_4(i, j, k, l)
		+ M1_r[2] * (
		...
		+ M1_r[4] * psi_1(i) * psi_1(j) * psi_1(k) * psi_1(l);
}

complex<double> DIE::M1_5(int i, int j, int k, int l, int m)
{
	return M1_r[1] * psi_5(i, j, k, l, m)
		+ M1_r[2] * psi_4(i, j, k, l) * psi_1(m)

		+ M1_r[2] * (psi_4(i, j, k, m) * psi_1(l) + psi_3(i, j, k) * psi_2(l, m))
		+ M1_r[3] * psi_1(m) * (psi_3(i, j, k) * psi_1(l))


		+ (M1_r[4] * psi_1(l) * psi_1(m) + M1_r[3] * psi_2(l, m)) * (
			psi_1(k) * psi_2(i, j)
			+ psi_1(i) * psi_2(j, k)
			+ psi_1(j) * psi_2(i, k)
			)
		...
		+ M1_r[5] * psi_1(i) * psi_1(j) * psi_1(k) * psi_1(l) * psi_1(m);
}

complex<double> DIE::M1_6(int i, int j, int k, int l, int m, int n)
{
	complex<double> result = M1_r[1] * psi_6(i, j, k, l, m, n) + M1_r[2] * psi_1(n) * psi_5(i, j, k, l, m)
		+ M1_r[2] * psi_5(i, j, k, l, n) * psi_1(m) + M1_r[2] * psi_4(i, j, k, l) * psi_2(m, n)
		+ M1_r[3] * psi_4(i, j, k, l) * psi_1(m) * psi_1(n);

	result += M1_r[3] * psi_1(n) * (psi_4(i, j, k, m) * psi_1(l) + psi_3(i, j, k) * psi_2(l, m))
		+ M1_r[2] * (
			psi_5(i, j, k, m, n) * psi_1(l) + psi_4(i, j, k, m) * psi_2(l, n)
			+ psi_4(i, j, k, n) * psi_2(l, m) + psi_3(i, j, k) * psi_3(l, m, n)
			)
		+ M1_r[4] * psi_1(n) * psi_1(m) * psi_3(i, j, k) * psi_1(l)
		+ M1_r[3] * (
	...
	result += (M1_r[5] * psi_1(l) * psi_1(m) * psi_1(n) + M1_r[4] * psi_2(l, n) * psi_1(m)
		+ M1_r[4] * psi_1(l) * psi_2(m, n) + M1_r[4] * psi_1(n) * psi_2(l, m) + M1_r[3] * psi_3(l, m, n))
		* (
			psi_1(k) * psi_2(i, j)
			+ psi_1(i) * psi_2(j, k)
			+ psi_1(j) * psi_2(i, k)
	...
	result += (M1_r[4] * psi_1(l) * psi_1(n) + M1_r[3] * psi_2(l, n)) * (
		psi_2(k, m) * psi_2(i, j) + psi_1(k) * psi_3(i, j, m)
		+ psi_2(i, m) * psi_2(j, k) + psi_1(i) * psi_3(j, k, m)
		+ psi_2(j, m) * psi_2(i, k) + psi_1(j) * psi_3(i, k, m)
		) + M1_r[3] * psi_1(l) * (
	...
	result += (M1_r[4] * psi_1(n) * psi_1(m) + M1_r[3] * psi_2(m, n)) * (
		psi_2(k, l) * psi_2(i, j) + psi_1(k) * psi_3(i, j, l)
		+ psi_2(i, l) * psi_2(j, k) + psi_1(i) * psi_3(j, k, l)
		+ psi_2(j, l) * psi_2(i, k) + psi_1(j) * psi_3(i, k, l)
		) + M1_r[3] * psi_1(m) * (
	...
	result += (M1_r[5] * psi_1(n) * psi_1(m) + M1_r[4] * psi_2(m, n)) * (
		psi_2(i, l) * psi_1(j) * psi_1(k)
		+ psi_1(i) * psi_2(j, l) * psi_1(k)
		+ psi_1(i) * psi_1(j) * psi_2(k, l)
		) + M1_r[4] * psi_1(m) * (
	...
	result += M1_r[6] * psi_1(i) * psi_1(j) * psi_1(k) * psi_1(l) * psi_1(m) * psi_1(n)
		+ M1_r[5] * (
